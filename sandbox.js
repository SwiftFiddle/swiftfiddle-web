...
Sandbox.prototype.execute = function(success) {
  ...
  exec(['sh', path.join(this.root_dir, "run.sh"), this.timeout + 's', '-v', path.join(this.root_dir, this.temp_dir) + ':/usercode kishikawakatsumi/swift-' + this.toolchain_version, 'sh', '/usercode/script.sh', this.command, this.options].join(' '));
